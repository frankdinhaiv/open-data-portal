...
- Each prompt card shows: prompt text only (no category chips — categories are leaderboard-only filters in P1-2)
...
### Mode Switching
- User selects mode from sidebar ("Đấu Trường", "Song Song", "Trực Tiếp") or from topbar dropdown
- Selected mode highlighted/active state
- Clicking mode clears current battle and resets to welcome/prompt selector
- If user is on leaderboard view, selecting a mode navigates back to arena view first
...
- Exactly 3 random prompts from seed dataset (prompt text only, no category chips)
...
- [ ] Suggested prompts display exactly 3 random prompts on welcome screen (text only, no category chips)
- [ ] Suggested prompts randomized on every load via `/api/prompts?random=true&count=3`
- [ ] Mode selector navigates back to arena view if user is currently on leaderboard
